...
CREATE TABLE members(
  national_id         NUMBER(4)     CONSTRAINT  m_nat_id_PK     PRIMARY KEY
                                    NOT NULL,
  ...
  national_id_payer   NUMBER(4)     CONSTRAINT  m_nat_id_pay_FK REFERENCES members(national_id)
                                    NOT NULL
);
...
country_id  NUMBER(4)     CONSTRAINT  country_id_PK PRIMARY KEY
                          NOT NULL,
country     VARCHAR2(30)
);
...
zip_code    NUMBER(4)     CONSTRAINT  zc_zip_code_PK          PRIMARY KEY
                          CONSTRAINT  zc_zip_code_CHK  CHECK(zip_code BETWEEN 1000 AND 9999)
                          NOT NULL,
city        VARCHAR2(30)  NOT NULL,
country_id  NUMBER(4)     CONSTRAINT  zc_cntry_id_FK          REFERENCES countries(country_id)
                          NOT NULL
);

REM **************************************************************** --OK housenumber => varchar2
...
CREATE TABLE addresses(
  ...
  zip_code    NUMBER(4)     CONSTRAINT  ad_zip_code_FK          REFERENCES zip_codes(zip_code)
                            CONSTRAINT  zc_zip_code_lengte_CHK  CHECK(zip_code BETWEEN 1000 AND 9999)
                            NOT NULL
);
...
CREATE TABLE address_members(
  national_id   NUMBER(4)     CONSTRAINT  am_nat_id_FK    REFERENCES members(national_id)
                              NOT NULL,
...
location_id     CHAR(4)       CONSTRAINT  location_id_PK  PRIMARY KEY
                              NOT NULL,
location_name   VARCHAR2(30)  NOT NULL,
location_info   VARCHAR2(30)  NOT NULL,
...
CREATE TABLE sports(
sport_type    VARCHAR2(20)  CONSTRAINT  sport_type_PK PRIMARY KEY
                            NOT NULL,
...
CREATE TABLE activities(
  ...
  partner               VARCHAR2(20),
  location_id           CHAR(4)       CONSTRAINT  ac_loc_id_FK    REFERENCES locations(location_id)
                                      NOT NULL,
...
CREATE TABLE reservations(
  national_id NUMBER(4) CONSTRAINT  re_nat_id_FK    REFERENCES members(national_id),
...
CREATE TABLE subscription_dates(
  ...
  inhabitant              VARCHAR(30) NOT NULL,
